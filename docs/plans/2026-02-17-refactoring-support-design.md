...
The VS Code C-Next extension currently provides 3 IDE providers (Completion, Hover, Definition) backed by a centralized `SymbolResolver`. The extension can find where symbols are *defined* but not where they are *used*. Adding refactoring support requires reference-finding as the foundational capability.
